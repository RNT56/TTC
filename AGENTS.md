...
15. `docs/OPERATIONS.md` before changing deployment topology, managed environments,
   ...
   exact D68 names and gates live in `infra/deployment/deployment-policy.v1.json`,
   while D69 image/runtime authority lives in
   ...
   leaves both upstream majors frozen; D76's private local storage/query/lifecycle
   fixture lives in `infra/observability/observability-custody-policy.v1.json` and
   leaves every upstream major frozen.
...
- the SEC-006 contract/fixture runtime evidence remains anchored at protected PR #31
  ...
  PR CI/security `29635355008`/`29635354985`, protected `f68314d`, and final
  post-merge CI/security `29635605328`/`29635605305` pass at contract/fixture
  maturity; no managed
  environment or live service is proven. Evidence PR #126 is protected at `f6f2620`
  with exact post-merge CI/security `29640292597`/`29640292585`. OPS-002 is now in progress on the D69
  ...
  format. PR #127 is protected at `a1d72ad`; correction PR #128 is protected at
  `f1d8850`. All 47 correction gates, all twelve PR checks, and exact post-merge
  CI/security `29644126620`/`29644126611` pass. Protected publication run
  `29644408106` and independently downloaded final artifact `8429638868` bind three
  digest-only GHCR objects, nonempty SPDX documents, zero fixed low-or-higher
  findings, attached GitHub provenance, exact public manifest hashes, exact pulled
  config identities, and a green same-artifact runtime smoke to source `f1d8850`.
  Publication record `132dc5f4…fd72` verifies with every managed-sandbox, rollback,
  live, production, and external-beta claim false. OPS-002 remains in progress until
  those exact references are installed, upgraded, rolled back, and corrected forward
  ...
  all 258 worker tests in the complete pinned training/MJX/co-design environment,
  ...
  durable spool exists; failure cannot change product authority. Eleven focused
  ...
  clean protected source and exact D61 evidence. D57
  evidence-
...
- D59/P9-002/P9-003 is protected at local controlled-engine-smoke maturity through
  ...
  3.9.0 short rollouts. Worker readback recomputes patch, candidate, native-evidence,
  and Pareto truth. The release-binary focused smoke admits 2/3, returns one Pareto
  point, and passes the 50 ms tier-0 SLO in repeated release-binary runs. All 41
  required local gates pass under
  Python 3.12.7 with 233 worker tests, 74 Gateway tests, and 39 Studio tests. Tier 3
...
- D60/P9-002 is protected at
  ...
  seed/manifold, replace-only patch hashes, unique candidate hashes, and deterministic
  replay. The request field set is exact and the outer/embedded JSON is byte/depth/
  node bounded before algorithm execution; independent Node readback recomputes the
  manifest, patch, parameter, candidate, plan, and cache-key authority. Its bounded-
  diversity loss has no physical or engine feedback. Validator,
  Rapier, MuJoCo, physical constraints, candidate admission, Pareto, overnight-200-
  ...
  in progress. D61 is the separate engine-consumption lane and must not promote the
  plan;
- D61/P9-002/P9-003 is protected at platform-scoped
  `local-engine-200-batch` maturity through PR #111/`1c37567`. Internal batch/
  evidence 1.0.0 deterministically replay one exact D60 plan, consume its 200 hashes,
  checkpoint every candidate, fence interruption, prove zero-dispatch cancellation,
  and expose Pareto/finalists only when complete. Protected artifact `8396554544`
  evaluates 200 native and 123 eligible Rapier/MuJoCo rows, admits 87, returns four
  Pareto points, and selects three tier-3-held finalists. All 43 local and protected
  gates pass with 242 worker, 74 Gateway, and 39 Studio tests. D62/P9-006 records that
...
- D64/P9-002/P9-003 is protected at catalog-backed, exact-runtime-scoped
  ...
  rows; v1 remains historical. Cache/checkpoint/resume/candidate records bind both
  catalog and D63 runtime authorities and refuse either foreign authority before
...
- D65/P9-003 is protected at exact-runtime-scoped
  ...
  to exact MuJoCo body-mass closure. Co-design batch/evidence 4.0.0 and
  `forge-codesign-training-authority/1.0.0` bind the exact bundle/physics hashes per
  tier-2 row. Bench tables now fail closed unless their voltage grid covers the
...
- D66 is protected at `catalog-performance-grid-format` maturity through PR #119.
  ...
  surface/99-case corpus, the twentieth compatibility surface, generated artifact
...
- compatibility policy 1.0.0 is machine-checked across nineteen API/event/format/
  package boundaries; the D62 candidate additively registers internal runtime-
  authority 1.0.0 and coordinated plan/batch/evidence 2.0.0 formats without changing
  a public route, event, queue kind, CLI/WASM facade, or persisted database format;
...
- D51/P8-003 is protected through PR #89/`b5418ac` at local archive-inspection
  ...
  `forge-recorder-inspection/1.0.0` self-consistency summary. Studio invokes it via
  exact `@tauri-apps/api` 2.11.1, rejects response/authority drift, uploads no frames,
...
- D53/P8-003 is protected at local private-object-integrity maturity through PR #93/
  ...
  gateway completion HEAD-verifies all five and reads bounded manifest/receipt
  bindings. Migration 0025 and export 1.5 retain the evidence, but only object
  integrity may advance. Archive semantics, telemetry admission, recorded-device/
...
- D54/P8-003 is protected at local sovereign archive-semantics admission maturity
  ...
  separate admission plus object-backed telemetry reference. Migration 0026, user-
  data export 1.6, deletion, and explicit D45 object-backed training refusal cover
  the lifecycle boundary. D53 remains immutable; device/session authenticity,
  recorded-device/field provenance, sharing/training authority, production object
...
- D55/P8-002/P8-003 is protected at local read-only protocol-fixture maturity
  ...
  port. Raw UID/responses stay native and Studio receives only domain-separated
  hashes with all authenticity/device/recorded-device/field/sharing/training flags
  false. Nineteen native and twenty-eight Studio focused tests pass. This is a
...
- D56 is protected at local custody-fixture maturity through PR #100/`1bf127d`. It
  ...
  signoffs/artifact/model/two-port/D55 identity/nonclaim binding; and a create-new
  proof outside the archive after shell-owned pre-open/post-clean-receipt identity
  continuity. The proof binds the exact authorization-file and receipt digests and
...
- controlled hardware evidence remains D30/D12-only: rover before quad, local
  ...
  before any device claim. D55's two stable MSP identity passes remain self-reported
  and unattested; retain them as observation evidence only. D56 can bind a separately
  signed acceptance authorization to native recorder start/stop continuity, but its
  proof remains an acceptance-authority record, not a device signature or archive-v1
  promotion. Require a real deployment root, named-hardware/operator/lab evidence,
  rotation/revocation and suspend proof before any custody maturity beyond fixture;
  recorded-device and field claims still require later reviewed authority;
...
| Deployment/runtime/observability | `pnpm verify:deployment`; `pnpm verify:hardened-runtime`; `pnpm verify:hardened-registry`; `pnpm verify:observability`; warning-free Compose configuration render with all explicit inputs; staged-source ownership/mode and effective supplemental-group inspection; Gateway/worker secret, startup, readiness, termination, correlation, redaction, and refusal tests; bounded batch/line/timeout/endpoint/redirect/non-2xx/no-retry/no-spool/authority-isolation tests for transport changes; image build/SBOM/provenance/vulnerability/runtime smoke in CI; protected-main manual digest-only publication plus registry-manifest hashing, attached-attestation verification, independent pull, exact-image smoke, and downloaded evidence; full `pnpm verify`; retained managed-sandbox install/upgrade/rollback/corrected-forward evidence before OPS-002 closure and authenticated external collector, access/audit, availability/failure, retention/deletion/export/residency, real backend/trace/dashboard/alert evidence before OPS-003 closure |
| Workers | Python 3.12 environment; `pnpm --dir workers test`; live-adapter contract tests when touched; for D59/P9 co-design install exact `workers[dev,mujoco,training,codesign]`, build release `forge-validate`, run `pnpm codesign:engine-smoke -- --require-tier0-budget`, bind an exact gateway-owned admitted snapshot, refuse caller manifolds and >9 controlled candidates, recompute replace-only patches/candidate/native-evidence/Pareto hashes at worker readback, require real Rapier 0.33.0 plus MuJoCo 3.9.0 evidence, keep the <50 ms tier-0 SLO separate from validator admission, hold tier 3, and retain every overnight/trained-finalist/catalog/provider/build/hardware/field nonclaim; for D60 run focused search-plan tests plus clean exact-source `pnpm codesign:search-plan`, require pinned `cmaes==0.13.0`/`optuna==4.9.0`, exact 100/100 allocation, frozen mixed manifold/seed, 200 unique reapplied candidate hashes, deterministic replay, and all physical/engine/admission/Pareto/overnight/training/catalog/provider/build/hardware/field nonclaims; for D61 run focused batch tests plus clean exact-source `pnpm codesign:engine-batch`, require deterministic D60 replay, one contiguous exact-hash prefix, atomic per-candidate checkpoints, interrupted-attempt fencing, zero-dispatch cancellation, exact resume without prefix recomputation, sovereign native/Rapier/eligible-MuJoCo evidence, complete-only admission/Pareto and three tier-3-held finalists, measured-local-runtime fields, and false overnight/provider-billing/catalog/trained/build/hardware/field claims; D61 may promote neither D59's controlled smoke nor D60's proposal plan; for D62 run focused v2 plan/batch tests, clean exact-source plan and batch smokes, and `pnpm codesign:platform-compare` over clean same-protected-source artifacts from each supported architecture; require self-hashed OS/kernel/machine/libc/byte-order, Python, NumPy distribution/configuration/CPU/BLAS/LAPACK, and algorithm-distribution authority in every plan/cache/checkpoint/candidate; refuse foreign replay/evaluation/resume, keep heterogeneous resume and portable cache false, and grant no tier-3 authority from the comparison. P9-006 closes only after protected evidence and reconciliation establish either all-200 equality or this exact platform-bound scheduling/refusal contract; for D64 run focused v3 search/batch/native tests and exact-source plan/batch smokes with `FORGE_CATALOG_DIR`; independently recompute sorted raw catalog and row hashes, require exact D5/D10/D32 choice/revision/review/license/export lineage, catalog-aware native v2, equipped-only proofs, synchronized inline capacity/discharge, and catalog/runtime cache/checkpoint refusal; keep marketplace review/live persistence, catalog-native MuJoCo physics, tier 3, overnight/provider, build, hardware, and field claims false; for native training install exact `workers[dev,mujoco,training]`, run both PPO/SAC focused tests plus `pnpm training:smoke`, inspect runtime pins/source/lockfile/dependency-manifest/contract/config/model digests and honest nonclaims, and run the pinned local `pip-audit`; for P7-009 additionally run `pnpm training:offline-smoke`, require one gateway-owned consented source log and exact tape/dataset/warmstart hashes, reject task/tensor/truth/action/timestamp/sample/provider substitution, prove the frozen BC-to-randomized-PPO stages plus optimizer update and same-seed dataset/warmstart/ONNX digests, apply the unchanged scorecard, and state controlled-synthetic versus recorder/device/field maturity; for P7-012 additionally prove tensor-v1/v2 execution/refusal, exact device authority with no fallback, frozen recipes/thresholds/seeds, all baseline/mass/Kv/wind rows, atomic interruption/resume, request-hash and ONNX size/digest/export validation, safe hardware metadata, separate simulated-vehicle and host-energy fields, cost/nonclaims, exact retained policy bytes, clean protected source, and downloaded evidence; for P7-010 install `workers[dev,mujoco,training,mjx]`, retain `pnpm sim:mjx:feasibility` as the immutable v1 harness check, and follow `docs/MJX-DECISION.md` for v2: exact ordered proxy/authority hashes, 12-hour scorecard and 200-candidate budget artifacts, current provider USD/hour evidence, clean protected source, float64, resolved CUDA/ROCm GPU or TPU with fallback forbidden, all compile/sample/parity/cost-normalized measurements, centralized verdict/blockers, and independent retained hashes; CPU/Metal/reference feasibility never authorizes a decision; for P7-013 install exact `workers[dev,mujoco,training,deployment,queue]`, test the exact deployment contract/evidence validator, CUDA refusal and no-fallback authority, provider-call persistence/cancellation/recovery, shared quota/debit/refund transactions, and migration 0024; a real sandbox close additionally requires a clean protected revision, immutable deploy/function/image identity, successful L4 run, billing/tag reconciliation, alert/SLO delivery, cancellation and late-result refusal, application-artifact deletion, verified automatic provider-call expiry within seven days, recovery without duplicate output, and validation by `forge-modal-training-sandbox-evidence/1.0.0`; D38 lease/retry/cancellation/timeout/duplicate/crash matrix for queue changes; for policy delivery prove lease checks before and after upload, one job/one policy, byte-free persistence, digest substitution refusal, cancellation-without-authority, and exact object readback |
| D65 catalog training/co-design | Build debug `forge-validate`; run focused Rust bundle/CLI and Python bundle/batch tests, exact-source `pnpm codesign:engine-batch`, and compiled MuJoCo mass readback. Require historical bundle v3/physics v1/batch v4 authority, complete catalog and equipped-row hashes, sourced catalog mass/inertia at every declared mount, independent uniform-solid inertia/COM readback, exact MJCF mass closure, bounded review/license/fallback fields, and exactly one applicable bench table whose voltage grid covers the battery range and diameter×pitch matches the equipped prop. Retain rejected tables/reasons, fail closed on ambiguous tables, never edge-clamp or implicitly merge separate single-voltage tables, bind per-rollout bundle/physics hashes, and keep applicable-catalog-thrust, tier 3, overnight/provider, marketplace/live-catalog, build, hardware, and field claims false. D66 format support alone does not change D65 authority; D67 owns any exact-grid/curve-readback consumption. |
| D66 catalog performance-grid format | Run focused Rust generic-thrust and file-catalog corpus tests plus Python ETL/corpus parity; require exact markerless/explicit v1 and v2 reads, unsupported-major/mixed-shape refusal, finite positive per-point voltage, bounded 2–10,000-point/64-table rectangular grids, unique coordinates, exact throttle `[0,1]`, nondecreasing thrust/current per voltage, stable table IDs, prop, positive confidence, and HTTPS source. Run compatibility/generated-doc/golden gates. Run `pnpm db:migrations:test` and full `pnpm verify:db`; prove migration 0027 on every populated predecessor, exact `legacy-unattributed` v1 preservation without fabricated metadata, distinct table identities at shared coordinates, and incomplete-v2-authority refusal. State explicitly that the checked-in row/data and D65 training authority do not advance. |
| D67 exact-grid/curve readback | Build debug `forge-validate`; run focused Rust CLI and Python bundle/batch tests plus exact-source `pnpm codesign:engine-batch`. Require catalog bundle v4, catalog-physics v2, curve-readback v1, training-authority v2, and batch/evidence v5. Retain exact row major/table/prop/source/confidence/ranges and every SI voltage/throttle/thrust/current point. Independently validate grid shape/uniqueness/endpoints/monotonicity, refuse every fixed-point voltage lookup outside the measured axis, and reconstruct all 101 total-thrust/normalized-voltage/normalized-current values from the versioned bilinear/fixed-point recipe. Reject point, curve, recipe, table-selection, authority, cache, checkpoint, and resume drift. A rejected table requires null selection and `tableDriven=false`. Prove table-driven acceptance with controlled-synthetic test data only; do not mutate or promote a catalog row. Keep sourced/applicable thrust, owner review, tier 3, overnight/provider, marketplace/live-catalog, build, hardware, field, and external claims false. |
...
| Desktop/hardware | scaffold tests plus `pnpm verify:desktop-native`; locked Desktop Cargo fmt/Clippy/tests; D30/D12 gate tests; no-auto-arm/props-off physical-confirmation/supervisor assertions; for D48 config writes prove exact schema/firmware version/command/range/hash, 115200 baud, OS-enumerated-port refusal, real pseudo-terminal bytes, and receipt-v1 nonclaims; for D49 prove bounded two-session stable-2025.12.x handshake, set/save acknowledgement, reconnect, repeated reported-identity hash, exact readback, SHA-256 binding of the four authoritative raw response streams, v2 receipt semantics, malformed/ambiguous/timeout/partial-state refusal, and CLI-arming-disabled state; execute the exact protocol on the named props-off FC with retained controlled-lab evidence before physical-device or lab claims; for D50/P8-013 prove independent archive/frame/receipt v1 versions, exact consent/D30/D12/OS-enumerated 115200-baud gates, one exclusive in-shell thread, bounded exact artifact/sequence/increasing-time object frames, no-overwrite append-only storage, sparse byte-offset index, drain/flush/sync before replay/receipt, exact frame/index/replay hashes, empty/partial/drift/cap/interruption refusal, private/no-training/no-auto-arm/false-device-attestation semantics, and a real pseudo-terminal round-trip; for D51 import require exactly five canonical non-symlink regular files, strict manifest/receipt versions/fields/privacy, actual aggregate bounds, streaming frame/index validation, exact sparse offsets/final entry/count/duration, frame/index/reconstructed-and-retained-replay hash equality, unsupported/non-canonical/tampered refusal, strict Studio command/response tests, no upload, and visible self-consistency/non-authenticity/device/field/sharing/training nonclaims; for D52 controls prove exact control/receipt versions and field sets, shell-owned `inactive|recording|finished` state across webview reloads, admitted-report-only contract/lockfile/seed authority, D30/D12/consent/new-absolute-path/OS-enumerated-115200-port enforcement in Studio and native code, bounded status/port/environment parsing, browser fail-closed behavior, explicit stop/error collection, and invariant private/no-training/no-device/no-field/no-auto-arm semantics; for D53 prove path-free plan/upload/materialization versions, exact five file roles/types/sizes/hashes, native same-origin HTTPS-or-loopback PUTs with redirects/default proxy disabled and sized streaming bodies, gateway owner/private/staged/HEAD/readback/cross-binding behavior, migration state constraints, export/deletion coverage, and permanent false archive/device/field/sharing/training authority; for D54 prove exact five-owner-object streaming size/SHA-256 enforcement, bounded timeouts, private exclusive temporary files and cleanup before persistence, native canonical archive replay, exact report/D53/model/contract/lockfile binding, separate immutable D53/D54 rows, byte-free bounded telemetry references, export/deletion, and refusal from legacy D45 training even with consent; for D55 prove exact probe/adapter versions and field sets, D30/D12/props-off/OS-enumerated-115200-port plus atomically inactive-recorder enforcement, MSP-v1 commands exactly 1/2/3/4/5/160, bounded framing/direction/command/checksum/deadline parsing, protocol 0/API 1.47/BTFL/stable-2025.12.x/KAKUTEH7/UID requirements, two byte-stable passes on one open port, raw-UID/response confinement, domain-separated hashes, and permanently false authenticity/device/recorded-device/field/sharing/training authority; for D56 prove exact trust-bundle/authorization/proof 1.0.0 field sets, deployment-only absolute path and SHA-256 pin, public-key-only purpose/validity/revocation/key-count bounds, strict Ed25519/weak-key/signature/time/revision/evidence/signoffs/artifact/model/two-port/D55/nonclaim refusal, shell-owned atomic pre-open and post-clean-stop observations, separate OS-enumerated ports and exact descriptor/identity/UID continuity, canonical receipt/archive revalidation, proof create-new outside archive v1, post-failure archive preservation, raw signature/key/UID confinement, strict Studio parsing, and permanent false device-cryptographic/recorded-device/field/sharing/training authority; for D57 prove exact ghost schema/frame/compact row layout, finite strict ≤600-second/100,000-source/6,001-render-point bounds, Euclidean divergence, endpoint-preserving decimation, monotonic preceding-point seek index, incomplete/duplicate/non-finite/version/layout/index/authority refusal, controlled-synthetic versus unverified maturity, static path precomputation, 60 Hz indexed-seek budget, production-browser play/frame-step interaction, no raw replay JSON/JSONB, and permanently false device/recorded-device/field authority; for D58 prove exact ladder/control versions and field sets, one shell-owned `inactive|rehearsing|rehearsal-complete` session, D12/safe-ID/hash/exportable-policy/passing-supervisor bindings, exact D9 rates/fallback, contiguous stage prefix, exact start/advance/end interactions, parallel/skip/substitution refusal, browser fail-close, Studio response rebinding, no hardware I/O, and permanently false physical/deployment/hardware/device/field/external-beta authority; never call local capture, controls, inspection, materialization, D54 semantic admission, D55 self-reported identity, D56 fixture custody, D57 fixture ghost geometry, or D58 rehearsal acknowledgments cryptographic device attestation, host-suspend, lab/field, real-twin/system-ID, sharing/training authority, recorded-device proof, physical/deployment evidence, or full named-hardware render performance |
...
15. Operational telemetry is allowlisted and versioned. Never log raw queries,
    headers, bodies, prompts, provider/error text, personal data, telemetry/model
    bytes, presigned URLs, or secret references; client IDs are not audit authority,
    and high-cardinality identifiers are never metric labels.
