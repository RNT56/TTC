...
| D65 catalog training/co-design | Build debug `forge-validate`; run focused Rust bundle/CLI and Python bundle/batch tests, exact-source `pnpm codesign:engine-batch`, and compiled MuJoCo mass readback. Require bundle v3/physics v1/batch v4 authority, complete catalog and equipped-row hashes, sourced catalog mass/inertia at every declared mount, independent uniform-solid inertia/COM readback, exact MJCF mass closure, bounded review/license/fallback fields, and exactly one applicable bench table whose voltage grid covers the battery range and diameter×pitch matches the equipped prop. Retain rejected tables/reasons, fail closed on ambiguous tables, never edge-clamp or implicitly merge separate single-voltage tables, bind per-rollout bundle/physics hashes, and keep applicable-catalog-thrust, tier 3, overnight/provider, marketplace/live-catalog, build, hardware, and field claims false. D66 format support does not change D65 authority; require a new exact-grid/curve-readback bundle/physics major before a v2 grid drives training. |
