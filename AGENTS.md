...
| Deployment/operations | `infra/deployment`, `infra/docker`, `infra/compose.hardened.json`, `scripts/deployment-policy.mjs`, `scripts/hardened-runtime.mjs`, `docs/OPERATIONS.md` | D68 environment/manifest/promotion and D69 image/runtime authority; build once, exact artifacts, file-mounted secrets staged outside the checkout as `root:10999`/`0440` for the single-host profile (Compose service-level `uid`/`gid`/`mode` are unsupported here), private topology, least privilege, forward-only migrations, no managed/live claim from contract or CI evidence |
...
| Deployment/runtime | `pnpm verify:deployment`; `pnpm verify:hardened-runtime`; warning-free Compose configuration render with all explicit inputs; staged-source ownership/mode and effective supplemental-group inspection; Gateway/worker secret, startup, readiness, termination, and refusal tests; image build/SBOM/provenance/vulnerability/runtime smoke in CI; full `pnpm verify`; a protected immutable registry artifact plus retained managed-sandbox install/rollback evidence before OPS-002 closure |
