...
15. `docs/OPERATIONS.md` before changing deployment topology, managed environments,
   ...
   publication authority lives in `infra/deployment/hardened-registry.v1.json`.
...
- the SEC-006 contract/fixture runtime evidence remains anchored at protected PR #31
  ...
  latest verified protected descendant is D70/OPS-002 correction PR #128 at `f1d8850`.
  ...
  under a real active D68 sandbox manifest. D66 implementation PR
...
| Deployment/operations | `infra/deployment`, `infra/docker`, `infra/compose.hardened.json`, `scripts/deployment-policy.mjs`, `scripts/hardened-runtime.mjs`, `scripts/hardened-runtime-registry.mjs`, `docs/OPERATIONS.md` | D68 environment/manifest/promotion, D69 image/runtime, and D70 digest-only registry publication authority; build once, exact artifacts, file-mounted secrets staged outside the checkout as `root:10999`/`0440` for the single-host profile (Compose service-level `uid`/`gid`/`mode` are unsupported here), private topology, least privilege, forward-only migrations, no managed/live claim from contract, image publication, or CI evidence |
...
| Deployment/runtime | `pnpm verify:deployment`; `pnpm verify:hardened-runtime`; `pnpm verify:hardened-registry`; warning-free Compose configuration render with all explicit inputs; staged-source ownership/mode and effective supplemental-group inspection; Gateway/worker secret, startup, readiness, termination, and refusal tests; image build/SBOM/provenance/vulnerability/runtime smoke in CI; protected-main manual digest-only publication plus registry-manifest hashing, attached-attestation verification, independent pull, exact-image smoke, and downloaded evidence; full `pnpm verify`; retained managed-sandbox install/upgrade/rollback/corrected-forward evidence before OPS-002 closure |
