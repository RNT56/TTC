...
const policy = JSON.parse(readFileSync("infra/observability/observability-policy.v1.json", "utf8"));
const schema = JSON.parse(readFileSync("schema/forge-observability-event.schema.json", "utf8"));

function messages(candidate) {
  return validateObservabilityPolicy(candidate, schema).join("\n");
}

test("D71 observability policy and event schema are coherent", () => {
...
test("high-cardinality metric labels and unsupported maturity claims fail closed", () => {
  ...
  candidate.maturity.workerPropagation = true;
  ...
  assert.match(errors, /workerPropagation must remain false/);
