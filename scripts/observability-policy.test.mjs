...
const policy = JSON.parse(readFileSync("infra/observability/observability-policy.v2.json", "utf8"));
const schema = JSON.parse(readFileSync("schema/forge-observability-event.v2.schema.json", "utf8"));

function messages(candidate) {
  return validateObservabilityPolicy(candidate, schema).join("\n");
}

test("D72 observability policy and event schema are coherent", () => {
...
test("high-cardinality metric labels and unsupported maturity claims fail closed", () => {
  ...
  candidate.maturity.providerPropagation = true;
  ...
  assert.match(errors, /providerPropagation must remain false/);
