...
| Claim P8-013 recorder closure | **Go at protected local private-object-integrity maturity only** | PR #87/`d8afe7f`, exact PR/post-merge CI/security, reviewed-tree equality, and pseudo-terminal proof protect D50's archive/frame/receipt, background thread, replay/index/hash, refusal, exclusivity, caps, and no-overwrite behavior. D51 separately protects read-only Studio archive inspection, PR #91/`a8120ab` protects versioned shell-owned controls, and PR #93/`08d892f` protects five private checksum-bound objects plus object-integrity-only completion. Sovereign archive semantics, telemetry admission, real adapter/device, host suspend, lab/field, ghost/system-ID, sharing/training reuse, and recorded-device claims require separate evidence |
