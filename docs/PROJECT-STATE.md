...
Latest verified protected descendant: `7d63512` (PR #118; D65 evidence reconciliation)
...
D66 is implemented locally at `catalog-performance-grid-format-candidate` maturity
and is not protected evidence. File-catalog row 2.0.0 carries voltage on every point;
...
hardware, or field maturity is added. All 44 local non-database gates pass; clean
every-populated-predecessor database CI and protected proof remain open.

## 2. Current verified results

| Check | Result | Interpretation |
|---|---|---|
| D66 local catalog performance-grid candidate | All 44 local non-database gates pass under Python 3.12.13, including Rust fmt/Clippy/workspace tests, native/WASM/golden/generated docs, 39 Studio, 74 Gateway, 247 workers, Brief-25 25/25, packaging, training/offline/MJX, and unchanged D65 200/97/two-point/two-held behavior. The registered corpus reports nine surfaces/99 cases and compatibility 20/20. On a fresh PostgreSQL 16 cluster, migration 0027 preserves two populated rows/RPM exactly, installs the exact key, retains two valid table identities, and refuses incomplete v2 authority. Clean every-predecessor `verify:db`, PR, and protected evidence remain pending because the host's existing Docker volume failed before migration with a relmapper I/O error | proves local row-v2/read-v1/parser/grid/migration shape only. It creates no sourced grid, applicable thrust, downstream training authority, marketplace review, hardware, field, or external maturity |
...
| Git state | latest verified protected descendant is D65 evidence reconciliation `7d63512`; D65 implementation is `ad54ab3`, D64 is `609a70d`, D63 is `54385c2`, and all listed P7/P8/QA/DOC/release anchors remain green; annotated `v0.1.0` is published. D66 is a dirty local candidate only | PR #118 exact head `9add00b`, protected `7d63512`, reviewed/protected tree `cc58b7b`, PR CI/security `29570643224`/`29570643100`, and post-merge CI/security `29571176449`/`29571176349` pass |
...
| Claim D66 per-point-voltage format | **No-go for protected or propulsion authority; local candidate only** | all 44 non-database gates plus fresh-Postgres migration-specific proof pass, but clean every-predecessor database/browser, PR, protected-tree, and post-merge evidence are pending. Even after protection, format capability cannot be cited as sourced/applicable catalog thrust, a new D65 curve, training, review/marketplace, hardware, field, or external evidence |
...
Immediate refresh trigger: finish clean every-predecessor database proof, protect, and
reconcile D66; then source/review a voltage-covered, prop-matched grid or retain the
analytic fallback. Before use, create a new exact-grid/curve-readback bundle/physics
