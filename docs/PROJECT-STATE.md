...
| Desktop/hardware | fail-closed scaffold plus protected D48 versioned native serial transport, protected D49 bounded pre/post `2025.12.x` target/readback, and protected D50/P8-013 exclusive bounded background serial-JSONL recorder with append-only frames, sparse index, replay-v1 finalization, exact hashes, private defaults, and false device attestation over a real pseudo-terminal | bind the recorder to a reviewed real adapter and verified Studio import; execute D49/recorder seams on named D12 hardware; browser WebSerial/WebUSB; signed apps; ghost/system-ID; Link image; lab pilots; field logs |
