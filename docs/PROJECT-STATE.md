...
clean local implementation evidence, not protected acceptance: protection, one
Linux/Apple pair from the same protected source, independent all-200 comparison,
and evidence reconciliation remain required.

## 2. Current verified results

| Check | Result | Interpretation |
|---|---|---|
| D62/D63 local exact-runtime-authority v2 candidate | Focused plan/batch tests pass 11/11; all 44 local gates pass under Python 3.12.13 with 244/244 workers, 74/74 Gateway, and 39/39 Studio. The v2 planner and scheduler bind one self-hashed numeric runtime authority into plan/cache/checkpoint/candidate lineage, reject foreign workers before work or resume, and keep heterogeneous resume false. Clean implementation evidence at `092af38` evaluates 200 native/125 eligible Rapier-MuJoCo rows, admits 89, returns four Pareto points, and retains three tier-3-held finalists; evidence files hash to `0fca560f…8f64`/`cb869c7f…7938`. The independent comparison command checks all 200 patch/candidate hashes and grants no portability or tier-3 authority | proves the complete clean local fail-closed v2 candidate shape only. It is not protected, same-protected-source cross-platform, overnight/provider, catalog-choice, trained-finalist, build, hardware, field, or external evidence. P9-006 remains `[~]` until exact protected-source Linux and Apple evidence is compared and reconciled |
...
| Claim D62 exact-runtime recovery closure | **No-go; clean local D63 candidate only** | 11/11 focused tests, all 44 local gates, and clean Apple-arm64 implementation evidence at `092af38` prove the local shape, not acceptance. Require protected PR/post-merge proof, exact Linux/Apple artifacts from the same protected source, independent all-200 comparison, and evidence reconciliation. Even after closure, exact-platform authority permits only matching-runtime replay/resume; it does not grant portable cache, heterogeneous resume, tier-3, overnight/provider, or trained-finalist authority |
