...
Latest verified protected runtime descendant: `4ad8c8b` (PR #112; D61 evidence reconciliation)
...
is not a portable recovery format. Tier 3, heterogeneous scheduling, and overnight/
provider authority are blocked until coordinated v2 semantics either make every hash
cross-platform identical or bind platform/runtime and enforce same-platform resume.
Catalog choices, build, hardware, field, and external acceptance also remain open.

D63 defines the local D62 recovery candidate without pretending the numerical drift
was eliminated. Coordinated internal search-plan/evidence and engine-batch/evidence
...
Gateway, and 39 Studio tests. A dirty Apple-arm64 diagnostic executes 7 + cancel + 193, evaluates 200
native and 125 eligible Rapier/MuJoCo rows, admits 89, returns the same four-point
front, and retains the same three tier-3-held finalists. Its search-plan SHA-256 is
`7b77693d…603f`, batch result SHA-256 is `39b9bea7…dc2e`, and runtime-authority
SHA-256 is `a99337a8…b312`. This is implementation diagnostics, not acceptance:
clean exact-source evidence, protection, one Linux/Apple pair from the same protected
source, independent all-200 comparison, and evidence reconciliation remain required.

## 2. Current verified results

| Check | Result | Interpretation |
|---|---|---|
| D62/D63 local exact-runtime-authority v2 candidate | Focused plan/batch tests pass 11/11; all 44 local gates pass under Python 3.12.13 with 244/244 workers, 74/74 Gateway, and 39/39 Studio. The v2 planner and scheduler bind one self-hashed numeric runtime authority into plan/cache/checkpoint/candidate lineage, reject foreign workers before work or resume, and keep heterogeneous resume false. A dirty Apple-arm64 diagnostic evaluates 200 native/125 eligible Rapier-MuJoCo rows, admits 89, returns four Pareto points, and retains three tier-3-held finalists; the independent comparison command checks all 200 patch/candidate hashes and grants no portability or tier-3 authority | proves the complete local fail-closed v2 candidate shape only. The diagnostic is not clean-source, protected, cross-platform, overnight/provider, catalog-choice, trained-finalist, build, hardware, field, or external evidence. P9-006 remains `[~]` until exact protected-source Linux and Apple evidence is compared and reconciled |
...
| Git state | latest verified protected descendant is D61 evidence reconciliation `4ad8c8b`; D61 implementation is `1c37567`, D60 implementation is `71e7217`, D59 runtime is `fae00c5`, D58 runtime is `f7e7f57`, and all listed P7/P8/QA/DOC/release anchors remain green; annotated `v0.1.0` is published | PR #112 exact head `551419e`, reviewed/protected tree `5ef3a5d`, PR CI/security `29554043285`/`29554043269`, protected `4ad8c8b`, tree equality, and post-merge CI/security `29555056417`/`29555056481` pass. The D62/D63 branch is a local dirty candidate and has no protected authority |
...
| `pnpm verify` | protected D61 passes 43 required gates; the local D62/D63 candidate passes all 44 under Python 3.12.13 | generated contract-doc drift, migration policy, external acceptance, registered golden updates, Action pins, Rust fmt/Clippy/tests, WASM/schema/TS, 39 Studio tests, 74 Gateway tests with no skips, Brief-25 25/25, oracles, budgets, the governed 89-case boundary family, sim, packaging, pilots, 244 worker tests, four-task/offline/MJX/D59/D60/D61/D62 policy and v2 smokes, and patch hygiene pass. Clean exact-source/protected authority remains pending |
...
| `pnpm verify:compatibility` | protected D61 passes 19/19 top-level surfaces against policy 1.0.0; the local D62 candidate also passes after additive internal registration | D63 registers proposal-runtime authority 1.0.0 and coordinated plan/evidence/batch/evidence 2.0.0 inside the worker-artifact surface while preserving every public format, queue kind, API/event, recorder/custody, policy tensor, source, manifest, legacy, license, user-data, consent, lifecycle, and deprecation boundary |
...
| Claim D62 exact-runtime recovery closure | **No-go; local D63 candidate only** | 11/11 focused tests, all 44 local gates, and a dirty Apple-arm64 diagnostic prove the implementation shape, not acceptance. Require clean exact-source plan/batch evidence, protected PR/post-merge proof, exact Linux/Apple artifacts from the same protected source, independent all-200 comparison, and evidence reconciliation. Even after closure, exact-platform authority permits only matching-runtime replay/resume; it does not grant portable cache, heterogeneous resume, tier-3, overnight/provider, or trained-finalist authority |
...
The stable ledger currently contains **206 tasks: 149 done, 32 in progress, 24 open,
