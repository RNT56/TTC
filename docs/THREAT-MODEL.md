...
D76 separately revalidates one D75 signal set before private local persistence. The
fixture refuses non-absolute/checkout/permissive/symlink roots, creates owner-only
directories/files, publishes bounded objects and records through exclusive temporary
writes plus fsync/rename, caps live records at 128, and revalidates type/mode/owner,
length, SHA-256, schema, identity, and counts before exact queries. Manual or exact-
expiry deletion leaves a bounded receipt; non-mutating audit detects corrupt,
missing, orphaned, symlinked, temporary, conflicting, and incomplete state without
automatic repair. Generic CLI failures do not reflect attacker paths or input. Local
persistence still proves no authenticated transport, external collector, managed
custody, owner export, residency, HA, backup, telemetry backend, dashboard, alert,
managed, live, or production monitoring.
