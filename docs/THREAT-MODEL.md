...
| D55 stable MSP identity is observation, not attestation | firmware, board, build, and UID are unauthenticated self-reported replies; an emulator or replaying device can present one stable transcript; D56 can bracket it but does not make it device cryptography | protect D56, then obtain named D12 controller evidence, host-suspend tests, and controlled lab/field acceptance before any recorded-device or field claim |
