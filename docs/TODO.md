...
- [x] QA-001 — Added `pnpm verify` as the protected 33-step non-DB gate and `pnpm verify:db` for isolated Postgres/pgvector invariants. Both fail on missing prerequisites or stale generated/oracle/workflow/compatibility/archive artifacts and are documented in README/AGENTS. The golden-policy and final hygiene steps inspect the cumulative branch range so a clean checkout cannot vacuously pass *(updated through protected QA-008 PR #36 on 2026-07-13)*.
