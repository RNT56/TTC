...
- [~] QA-002 — Add browser E2E coverage for generate/draft/edit/validate/share/catalog/course/listing/job/maintenance flows using real built WASM and an isolated DB. *(2026-07-13 implementation candidate: `scripts/browser-e2e.mjs` drives all ten named flows through the production Studio bundle, built WASM, real gateway/validator artifact, anonymous share boundary, and migrated Postgres; `pnpm verify:db` and the required Postgres job now own the gate and upload structured evidence/failure screenshots. Local typecheck/build/policy checks pass; exact isolated-DB PR and protected post-merge proof remain required before `[x]`.)*
