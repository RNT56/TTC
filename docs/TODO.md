...
- [~] P3-001 — Postgres schema: DDL live in `infra/migrations/0001_catalog.sql` (+ 0002 taxonomy seed); migration RUNNER + a live database remain (no Postgres in this environment; compose exists)
...
- [~] P3-004 — ETL worker v1: fetch → Claude extraction with per-field citations → OCCT tessellation → LOD chain → dedupe → license ledger entry → human review queue. *(2026-06-12: the OUTPUT format + review-queue semantics are live — `catalog/` rows with citations/confidence/review, loader-enforced; the fetch→Claude→OCCT pipeline itself is open and needs API keys + unblocked egress)*
...
- [x] P3-007 — Proof pair *(2026-06-12)*: EMAX ECO II 2207 1900KV + CNHL Black 4S 1500 100C as `catalog/components/` rows with **per-field citations** (confidence 0.7 + mandatory review notes — the env's egress allowlist blocks direct datasheet fetch, values transcribed from search-quoted manufacturer/retailer pages; P3-004 review-queue semantics). `examples/vx2-proof.forge.json` carries `rotors`+`battery` slots as semver refs with a pinned lockfile → **Admitted with `--catalog`, CTR-006-rejected without** (CI: verdict matrix runs with the catalog). `tests/proof_pair.rs`: baked AABB within 1 % of cited dims, resolver pins, CAT-compatible pair, review-note enforcement. The frozen vx2-hornet translation cannot gain slots (vintage); the synthetic VX-2 Mini derivative is the proof body
