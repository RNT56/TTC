...
- [~] P10-005 — Leaderboards: per-course/archetype/class; replay verification — universally checkable (D17), server re-verified as anti-cheat hygiene (= XC-25) *(2026-06-15: leaderboard routes/tables live; `/v1/replays` persists server verification artifacts; leaderboard submissions compute verification server-side and reject blind client `verified` claims; Studio verified-board filters now slice by EnvSpec archetype, verification-header class, and verified/held status; `replay.verify` emits durable course/archetype/class dimensions; server-side dimension columns remain open)*
