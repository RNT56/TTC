...
- [~] P4-008 — BYO Anthropic key + metered credits plumbing (D3); studio-free tier boundaries *(2026-06-13: gateway accepts per-request BYO key via `x-forge-anthropic-key`/body or deployment `ANTHROPIC_API_KEY` for opt-in Anthropic generation; studio settings, account metering, and credit ledger pending)*
