...
- [~] SEC-006 — Threat-model auth/session/BYO-key/provider callbacks/object URLs/job payloads/SSRF/prompt injection/zip or model bombs and add negative tests/rate limits. *(2026-07-13: canonical threat model, pinned-origin Auth.js/CSRF boundary, header-only nonpersistent provider keys, production config failures, bounded JSON/network/process/object/archive behavior, prompt-injection containment, classed limiter, 59/59 gateway, 104/104 workers, archive tests, published v0.1.0 re-verification, and all 32 local gates pass. Local `verify:db` could not start because Docker reset the connection and reports VM filesystem inconsistency/no free extension space; exact-tree protected Postgres proof remains before `[x]`.)*
