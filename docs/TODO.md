...
- [~] QA-003 — Add accessibility, keyboard, focus, contrast, reduced-motion, responsive, and viewer-grade browser acceptance; publish the supported-browser matrix. *(2026-07-13 local candidate: production-bundle `scripts/browser-support.mjs` refuses implicit/skipped execution, records exact revision/Studio/engine versions, requires real hashed WASM and validator admission, and runs Chromium/Firefox/WebKit through semantic landmarks/names, skip/focus indication, keyboard orbit/equipped-variant/explode/blueprint, live announcements, AA contrast, critical-target, renderer, presentation-asset, and positive-draw assertions. Chromium uses full WebGL and also proves 390 x 844 containment/reduced motion; Firefox/WebKit use the core-baked Canvas2D schematic and do not load scene/Three.js chunks. Studio exposes semantic regions/labels/instructions/status, keyboard camera controls, 3 px focus, skip navigation, stronger muted contrast, 28 px critical controls, reduced-motion behavior, and explicit full-Studio/viewer-grade tiers. The required Postgres job installs all engines and uploads both browser evidence files. Protected PR/post-merge evidence is still required before `[x]`; WebKit/narrow proxies do not claim Apple/mobile devices, screen readers, external users, or field maturity.)*
