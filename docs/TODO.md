...
- [~] QA-004 — Test migrations from every supported prior schema with populated data; document backup, rollback/roll-forward, and failed-migration recovery. *(2026-07-13 implementation: D37 defines every exact checked-in pre-1.0 prefix as supported; the shared advisory-lock runner validates contiguous source/checksum history and transactionally binds each SQL file to its ledger row. `db:assert-migrations` covers a clean install, populated catalog/review/generation/platform/consent/lifecycle fixtures across prefixes 0001..0019, exact preservation/idempotency, deliberately reversed authority backfill, injected atomic failure plus corrected roll-forward, checksum/gap refusal, and concurrent apply-once behavior, with structured required-CI evidence. The local policy tests pass; protected Postgres, exact PR/post-merge CI/security, and evidence inspection remain before `[x]`.)*
