...
- [~] QA-010 — Create external acceptance scripts and evidence templates for builder, photoscan, training, course, lab, print, marketplace, and maintenance milestones. *(2026-07-13: a versioned eight-milestone registry, private out-of-repo run-pack generator, completed-manifest validator, generated milestone runbooks, 9/9 policy tests, 34th local verification step, and required CI check are implemented. The contract rejects incomplete templates as proof and requires exact revision/environment, role independence, authority/evidence hashes, all steps/evidence kinds, measurements, findings review, signoffs, limitations, honest pass/fail/stop semantics, secret exclusion, strict manifest shapes, and private-data lifecycle fields. Protected PR/post-merge proof and final ledger closure remain open; no EXT task is implied.)*
