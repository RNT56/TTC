...
- [x] REC-007 — The original 30-step recovery gate, Postgres gate, green PR checks, protected merges, final post-merge CI/security, and manual nightly are linked in `PROJECT-STATE.md`; protected main has the later 32-step gate, and this QA-008 candidate extends it to 33 steps *(updated 2026-07-13; QA-008 protected proof pending)*.
...
- [x] GOV-005 — All workflow Actions are pinned by immutable SHA; the workflow-pin step in protected main's 32-step gate and this 33-step QA-008 candidate, plus the required dependency-review job, reject mutable refs; workflows use explicit read-only defaults and narrow job grants; security/release generate SPDX SBOMs; and repository Actions policy allows GitHub-owned Actions plus only the seven reviewed third-party SHAs. The pinning and supply-chain proof is protected; only QA-008's added gate awaits protected proof *(updated 2026-07-13)*.
...
- [x] QA-001 — Added `pnpm verify` as the protected 32-step non-DB gate and `pnpm verify:db` for isolated Postgres/pgvector invariants; this QA-008 candidate extends the former to 33 steps. Both fail on missing prerequisites or stale generated/oracle/workflow/compatibility/archive artifacts and are documented in README/AGENTS. The candidate golden-policy step and protected final hygiene step both inspect the cumulative branch range so a clean checkout cannot vacuously pass *(updated 2026-07-13; QA-008 protected proof pending)*.
...
- [~] QA-008 — Define golden-artifact review/update procedure so intentional schema/render/physics drift cannot be casually re-pinned. *(2026-07-13: the candidate machine registry owns fourteen artifact families and protects itself; parent/current inventory union prevents same-patch removal from hiding drift; the fixed record directory and overlap rejection preserve evidence history and single ownership; the frozen prototype is hard-required immutable; exact registered paths require one new append-only record with matching ID/class and complete rationale, impact, before/after evidence, review focus, and task/decision references. Ten focused policy tests and all 33 local gates pass; required PR and protected post-merge proof remain before `[x]`.)*
