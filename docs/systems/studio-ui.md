...
(v3.0) · **Decisions:** D3, D4, D14, D15, D16, D32, D34, D41, D55, D58, D59
...
| P8 | D30 lab-gated hardware bridge; config-diff, telemetry/replay, supervisor, system-ID, crash/ghost metadata, replay artifacts, telemetry logs, and maintenance records render in job/artifact panels; per-log sharing grant/withdraw and explicit share action are available; protected Desktop serial target/readback exists and D50's native background-recorder/archive is protected at local recorder-integration maturity through PR #87/`d8afe7f`; D51 protects a Desktop-only path input, native streaming verifier, and strict bounded archive-self-consistency summary with device/field/sharing/training nonclaims through PR #89/`b5418ac`; D52 protects strict bridge/port/status/start/stop controls sourced only from the active admitted report through PR #91/`a8120ab` and keeps raw frames plus device/field/sharing/training authority outside Studio; D53 protects authenticated private-object staging, exact native Desktop streaming PUTs, and object-integrity/archive-semantics separation through PR #93/`08d892f`; D54 protects the explicit authenticated sovereign-verification action, exact report/model binding, and bounded object-backed telemetry reference with all broader nonclaims retained through PR #95/`f8efb6f`; D55 protects a strict Desktop-only props-off/read-only MSP adapter probe through PR #97/`370d214` and displays stable hashed identity while keeping cryptographic/device/field/sharing/training authority false. D56 protects an optional Desktop-only signed custody mode through PR #100/`1bf127d` with exact admitted-model/authorization/identity-port/proof-path inputs, separate start/stop props-off confirmations, strict proof parsing, and bounded trust/evidence/identity/receipt hashes plus explicit acceptance-authority/device/recorded-device/field/sharing/training nonclaims. D57 adds strict parsing of a compact ten-minute indexed `forge-ghost-overlay/1.0.0`, precomputed observed/predicted X/Z tracks, explicit 60 Hz play/pause/frame steps, live meter divergence, and visible controlled-synthetic/unverified plus device/field nonclaims. D58 adds a Desktop-only shell-owned rehearsal panel for the exact four-stage prefix, D9 rates, admitted-report/exportable-policy/passing-supervisor-shaped inputs, and exact transition interactions while every hardware/deployment/evidence/device/field claim remains false; browser builds show the same ladder and keep controls locked. Trust private keys, raw UID/signatures, inferred device signatures, and raw recorder frames remain forbidden. WebSerial/WebUSB, real-trust-root/named-device capture, real ladder transitions, real D54-to-twin ghost execution, named-hardware performance, and field proof remain open |
| P9–P12 | fixture co-design Pareto points, wear, crash, repair, and fleet outputs render in job details; co-design points can apply admitted JSON-Patch candidates through patch/re-bake and save admitted points as openable models through the model admission route. D59 adds protected strict display of controlled engine maturity, native tier-0 budget evidence, real Rapier/MuJoCo tier labels, and tier-3/nonclaim state without turning a three-candidate local smoke into optimizer or overnight authority. D60's real CMA-ES/TPE output is deliberately a proposal plan and does not enter Studio because it has no candidate admission, metrics, or Pareto authority. Platform panels cover credits, license ledger/export-policy visibility, editable course creation, `?course=<id>` course URLs, replay-verified leaderboard filtering, classroom assignment/submission, marketplace kind/status filtering with row-level usage/equip actions, listing/policy-listing submission, and moderation reports; D34 requires explicit per-model pattern contribution and account-level leaderboard publication grants before those actions; the artifact panel includes the maintenance twin dashboard with fleet counts, crash scrubber, wear cards, repair rows, reorder hints, and vendor/print handoff links. Exact-hash 200-candidate engine evaluation, trained-finalist Pareto depth, and live marketplace/provider economics remain open |
...
D52 keeps the recorder lifecycle out of React even though Studio renders the controls;
PR #91/`a8120ab`, reviewed tree `25be1d3`, and exact PR/post-merge CI/security protect
this local control boundary.
On mount and explicit refresh, the Desktop-only panel strictly reads native bridge and
shell status, enumerates ports only when hardware is enabled, and restores active
capture identity after a webview reload. Start requires an admitted report, copies
only its exact contract/lockfile hashes and seed, and additionally requires one D12
rig, selected OS port, new absolute path, bounded sample rate, and a per-log consent
checkbox. Stop remains available for recording or finished state so the shell can
drain or surface the recorder error. The panel never receives frame bytes, uploads a
directory, marks a device/field session verified, or grants sharing/training reuse.
Browser builds visibly declare the controls unavailable and fail closed before Tauri
invoke. Archive verification remains a separate read-only action below the controls.

D53 adds one authenticated action only after that exact local inspection. Studio asks
Desktop for a sanitized plan, submits no paths or raw frames to the gateway, receives
five private presigned object contracts, maps only their name/URL/header/size/hash to
the native uploader, and requests gateway completion after native success. Strict
client parsing binds every blob and PUT back to the prepared file and rejects any
semantic/device/field/sharing/training authority promotion. The result is displayed
as “gateway object integrity on / archive semantics off”; it is never inserted into
the legacy `telemetry_logs` UI or offered to sharing/training flows. Browsers and
anonymous sessions fail closed. PR #93/`08d892f`, reviewed tree `90d8cbf`, and exact
PR/post-merge CI/security protect this local private-object-integrity boundary; they
do not prove archive semantics, telemetry admission, device/field provenance,
sharing, training, lab, or field maturity.

D54 adds a second explicit authenticated action only after D53 reports complete
object integrity and Studio has an active owned admitted model. The client submits
only that model ID to the materialization-scoped admission route, strictly parses the
entire native verification report and admission/reference envelopes, and cross-checks
every plan hash, model ID, contract/lockfile hash, replay hash/count/duration, and
nonclaim. It renders “archive semantics verified” separately from object integrity
and states that self-consistency is not device authenticity or field provenance.
No frame enters React, sharing controls, or training launch inputs. A retry may return
the same admission; a different model or any report/authority drift fails closed.
This UI and client are protected at local semantic-admission maturity through PR
...
11-flow production-browser suite, and Chromium/Firefox/WebKit pass. Exact head
`4321eaa`, reviewed tree `673a50c`, PR CI/security
`29519984713`/`29519984764`, protected PR #97/`370d214` tree equality, and
post-merge CI/security `29520651520`/`29520651581` pass.

D58 adds one disclosure after the recorder controls without changing bridge, recorder,
adapter, custody, or artifact state. All runtimes render the four stages, their
requirements, D9's 50 Hz policy-advisory/200 Hz supervisor/fallback contract, and the
non-authority warning. Browser runtimes stop there and invoke nothing. In Desktop,
Studio refreshes the strict native status, sources report hashes only from the active
admitted report, selects an exportable policy bound to that model, accepts only an
exact passing supervisor-decision output, and requires an explicit rehearsal-start
checkbox. It sends bounded values to the native shell, then independently rebinds
every returned identity and contiguous transition. HITL/constrained/free each require
their exact labeled checkbox interaction; the shell remains sovereign and rejects a
skip or substitution. The panel never calls a hardware command and labels every
acknowledged stage as local rehearsal—not physical confirmation, deployment evidence,
or authorization. Typecheck/build, 37 Studio tests, all 40 local gates, the fresh
clean/25-predecessor database, 12 production-browser flows, and all three browser
engines pass; PR #104/`f7e7f57` protects the exact tree while real transition
evidence remains pending.

D59 extends only the existing job-output disclosure boundary. Studio strictly parses
the evaluation version, source maturity, optimizer/benchmark flags, tier evidence,
candidate lineage, Pareto membership, and the complete false nonclaim set before it
labels an output as controlled-engine evidence. It shows native tier-0 runtime versus
the 50 ms SLO and names Rapier 0.33.0/MuJoCo 3.9.0 while stating that tier 3 is held.
Existing apply/save actions remain admission-gated and do not inherit optimizer,
overnight, trained-policy, catalog, provider, build, hardware, or field authority.
Focused typecheck and 39 Studio tests pass locally; PR #106/`fae00c5` protects the
exact tree and reviewed/post-merge CI/security evidence.
