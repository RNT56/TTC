...
`contract` (EnvSpec schema) + gateway (registry/leaderboards) *(proposed)* · **Plan
refs:** §13, §8.5 · **Decisions:** D6
...
## 4. Leaderboards (P10-005; D6 makes them honest)

Per-course, per-archetype, and per-class (e.g. stock VX-2 vs open class). A run
submits its replay `{contract hash + lockfile, env, seed, input tape}`; the
**server replays bit-exactly** (`replay.verify` worker) before a time enters the
board (XC-25). Client times are provisional; verified times are the record.
...
Generation pipeline (env generation), harness (ENV checks), `engines/sim` (course
