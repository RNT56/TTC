...
- P9 protected controlled-engine seam followed by the live optimizer and full
  ...
  relative to repeated Linux-x86-64 hashes. None of these lanes supplies portable/
  heterogeneous recovery, overnight/provider billing, actual trained finalists,
  catalog-choice, or external evidence;
...
The current dependency-complete local P9 step is P9-006/D62: resolve the observed
Apple-arm64/Linux-x86-64 CMA-ES drift before training any finalist. Prefer a
coordinated D60/D61 v2 that yields identical all-200 patch/candidate hashes across
supported worker architectures; if the underlying library cannot support that bar,
version exact platform/runtime authority and enforce same-platform scheduling and
resume. Only then may actual tier-3 training/scorecards begin. Catalog-choice search
and provider scheduling/billing remain separately gated. Fast proposal or local
engine completion is not an overnight run.
...
| P9 | P9-002, 003, 006, conditional 005 after protected D59-D61 | P6/P7 evidence; cross-platform plan/recovery authority; accelerator/provider budget for overnight/tier-3 work | close D62 with all-200 cross-platform equality or exact platform-bound scheduling/refusal; then prove catalog choices, retained overnight/provider billing, and actual scorecard-passing tier-3 finalists |
