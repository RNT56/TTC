...
| P9 Co-design | ◑ *(2026-07-17: D59's exact-snapshot engine smoke, D60's 100-CMA-ES/100-TPE plan, D61's 200-row consumer, D63's exact-runtime recovery authority, and D64's exact catalog battery search are protected through PR #115/`609a70d`; clean D64 artifact `8399829664` evaluates 200/200, admits 197, and returns two held battery points. D65 locally adds bundle v3/physics v1/batch v4 catalog mass, sourced-dimension inertia, MuJoCo mass closure, and per-rollout table applicability. Its dirty-tree 200-row smoke admits 97 and again returns two held battery points; the 6S/5×4.6 table is correctly rejected for the 4S/5×4.3 fixture and named analytic fallbacks drive the curve. All 44 required local gates pass with 246 worker, 74 Gateway, and 39 Studio tests; D65 is not yet protected. Portable cache, heterogeneous resume, applicable catalog-thrust data, a separate `>=3` physical front, actual tier-3 scorecards, retained overnight/provider billing proof, and external acceptance remain open.)* | 4 wk |
...
- [~] Tier-0 candidate evaluation < 50 ms native; 200-candidate CMA-ES generation overnight at tier 2 *(D59's protected release-binary runs pass the separate sovereign native 50 ms SLO. D60 produces 200 real-algorithm proposals. Protected D61/D64 and local D65 each consume an exact 200-row plan with durable same-authority recovery; local D65 evaluates 200/200 through catalog-bound bundle v3. Retained overnight/provider schedule and billing remain open.)*
- [~] Every returned point is a fully admitted contract (validator as constraint oracle) *(D59 protects the small exact-snapshot constraint-oracle seam. Protected D61 derives its complete front only from admitted sovereign rows and keeps partial fronts empty. D63 protects exact numeric-runtime authority; protected D64 adds catalog-aware native admission and equipped-revision lineage. Local D65 adds catalog mass/inertia, exact MuJoCo mass closure, and fail-closed bench-table applicability to tier 2, but is not protected and supplies no applicable table or actual trained finalists.)*

Applicable catalog-thrust evidence also depends on a reviewed per-point-voltage file-
catalog row/loader version; the current one-voltage-per-table representation cannot
express a grid spanning a non-degenerate battery range.
...
persistence, applicable catalog-thrust data, protected D65 tier-2 evidence, tier-3
...
D65 is the local catalog-bound physics successor. Bundle v3 and catalog-physics v1
...
curve. The current file-catalog row format supplies one voltage per table, so a
reviewed per-point-voltage row/loader version plus compatibility/migration/corpus
proof is required before an applicable non-degenerate grid can be ingested; separate
single-voltage tables must not be merged implicitly. A dirty-tree local 7 + cancel +
193 smoke evaluates 200/200, admits 97, and
returns two held battery points. It is implementation evidence only until exact-head
PR/protected-main checks and a clean retained artifact are reconciled.
