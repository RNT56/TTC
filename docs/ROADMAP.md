...
security `29647532626` and CI `29647532654` pass. D72 is now protected at contract/
fixture maturity. It preserves the v1 Gateway reader, introduces event major 2,
persists trusted request/trace/parent context on jobs, creates one database-owned
UUIDv4 attempt/span row per D38 claim, emits bounded worker start/completion events,
closes retry/cancellation/expiry/terminal paths, and extends owner export to 1.7
without payload or secret content. All 48 local gates pass, including 85 Gateway and
all 258 fully enabled worker tests; a fresh isolated database passes all 28
migrations, every 27 populated predecessor, D38 fault/cancellation correlation,
lifecycle assertions, and all 12 production-browser flows. PR #135 exact head
`4bb4721` passed all twelve required checks in CI `29859593049` and security
`29859592862`; tree-identical protected squash `a17ff74` passed post-merge CI
`29860284729` and security `29860284861`. Provider/deployment/actor/Desktop
...
D73 is protected at contract/fixture maturity through PR #140/`90cc58c`. Event major
3 retains frozen v1/v2 reads and adds only two exact authority paths: a managed
Gateway/worker deployment ID
...
null, and provider/deployment IDs remain forbidden metric labels. PR #140 exact head
`283b43a` passed all twelve checks in CI `29868001992` and security `29868001971`;
tree-identical protected squash `90cc58c` passed post-merge CI `29868693418` and
security `29868693478`. This cannot close provider delivery, deployment health,
telemetry backend, dashboard, alert, managed, live, production, or external-beta
criteria.
...
Evidence PR #145 exact head `f72a18f`, all twelve checks in CI `29878662250` and
...
| P9 Co-design | ◑ *(2026-07-17: D59's exact-snapshot engine smoke, D60's 100-CMA-ES/100-TPE plan, D61's 200-row consumer, D63's exact-runtime recovery authority, D64's exact catalog battery search, D65's catalog-bound tier-2 physics, D66's row-v2/read-v1/persistence compatibility, and D67's exact-grid/readback consumer are protected through PR #121/`08e880b`. Protected artifacts `8402573520`/`8405061774`/`8407177912` bind the catalog-physics, every-predecessor migration/browser, and 200/97/two-point/two-held readback/recovery boundaries. D67 advances to bundle v4/physics v2/readback v1 and batch/training-authority v5/v2: every SI grid point is retained and Python reconstructs all 101 curve points before table-driven authority; a controlled-synthetic grid proves acceptance/tamper refusal. The actual 6S/5×4.6 row remains v1, rejected for the 4S/5×4.3 fixture, and analytic fallbacks still drive the protected 200/97/two-point/two-held smoke. Portable cache, heterogeneous resume, sourced and owner-reviewed applicable catalog thrust, a separate `>=3` physical front, actual tier-3 scorecards, retained overnight/provider billing proof, and external acceptance remain open.)* | 4 wk |
...
D67 is the protected downstream consumer successor through PR #121/`08e880b`.
Bundle v4/physics v2 retain every
...
the real row remains v1/inapplicable and protected artifact `8407177912` returns
200/97/two-point/two-held under analytic fallback after exact recovery. Sourced and
owner-reviewed applicable data, tier 3, course-conditioned `>=3`, and provider/
...
D67 protects that authority through PR #121/`08e880b` as bundle v4, catalog-physics
v2, curve-readback v1, training-authority v2, and batch/evidence v5. It retains exact
row/table metadata and every SI grid point, independently reconstructs all 101 curve
points in Python, and partitions recovery by the resulting training authority. The
controlled-synthetic test is not catalog data. The next data step remains a new
immutable sourced and owner-reviewed component revision whose voltage range and prop
match the equipped design and whose exact row passes protected D67 before tier 3.
