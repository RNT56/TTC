...
maturity; recorder controls, gateway materialization, real adapter/device, suspend,
and field proof remain owned by the adjacent P8 tasks.
...
local dependency is recorder status/start/stop controls, followed by separately
versioned object-backed gateway import/materialization; real adapter/device and
suspend execution proceed only under the separate D12 lab lane.
