...
| P8 Bridge + Desktop | ◑ *(2026-07-16: D30 accepts controlled D12 lab pilots. D48/P8-012 now closes deterministic native serial transport at local integration maturity: the exact versioned Betaflight 2025.12/D12-quad/failsafe-only artifact is independently validated and hash-bound, serialport-rs restricts writes to an OS-enumerated port at 115200 baud, a real pseudo-terminal proves exact bytes, and the receipt refuses target-version/application claims. Config-diff, telemetry ingest, supervisor, sysid, replay/telemetry/maintenance readers, Studio artifact rows, and gateway/Desktop lab gates also exist. Real FC handshake/readback, WebSerial/WebUSB, signed Desktop delivery, recorder/ghost, Link image, lab pilots, and field evidence remain open.)* | 5–7 wk |
...
D48 closes P8-012 at deterministic/native transport integration maturity. Desktop
...
the exact bytes, but the receipt explicitly leaves target firmware and application
unverified. Real target handshake/readback, WebSerial, HITL, and lab/field evidence
