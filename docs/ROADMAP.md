...
| P9 Co-design | ◑ *(2026-07-16: D59's exact-snapshot three-to-nine-candidate sovereign native/Rapier/MuJoCo smoke is protected through PR #106/`fae00c5`; it admits 2/3, returns one Pareto point, and passes the 50 ms tier-0 SLO within all 41 protected gates. D60 now has an unprotected but 42-gate-green local plan that executes 100 pinned CMA-ES plus 100 pinned Optuna TPE proposals with deterministic hashes and no physical/engine feedback. Protecting D60, catalog choices, ≥3 admitted Pareto points, evaluating 200 exact hashes through engines overnight, trained finalists, and provider proof remain open.)* | 4 wk |
...
Exit criteria:
- [~] "Lightest quad for this course under constraints" returns ≥ 3 admitted Pareto points overnight *(the keyless 200-candidate constrained-course shape proof remains live. D59 adds a controlled three-candidate engine smoke with two admitted candidates and one recomputed Pareto point. D60 adds 200 real-algorithm proposals with no physical/engine feedback. Neither is a course-conditioned ≥3-point overnight result.)*
- [~] Tier-0 candidate evaluation < 50 ms native; 200-candidate CMA-ES generation overnight at tier 2 *(D59's repeated release-binary runs pass the sovereign native 50 ms SLO and exercise real Rapier 0.33.0 plus pinned MuJoCo 3.9.0 for three candidates. D60 executes 100 pinned CMA-ES plus 100 pinned Optuna TPE proposals, but does not evaluate them. A 200-candidate engine consumer, provider scheduling, and retained overnight evidence remain open.)*
- [~] Every returned point is a fully admitted contract (validator as constraint oracle) *(D59's protected controlled engine result recomputes every replace-only candidate snapshot and native-evidence hash, admits through the sovereign validator/Rapier/MuJoCo ladder, and derives Pareto only from admitted points. Full optimizer breadth and trained finalists remain open.)*
...
provider, build, hardware, or field work. Protection is still pending, and even a
protected plan will not close P9-002/P9-003 without a separately versioned engine
