...
current protected `f220d25` additively verifies 76 routes, two event families,
fifteen compatibility surfaces, and sixteen worker families through the same drift
gate. PR #54 restored the GOV-003 audit client
...
| P7 Training service | ◑ *(2026-07-15: P7-003 is protected through PR #64/`d1c4c38`: the controlled exact-pinned CPU MuJoCo/SB3 hover runtime owns admitted snapshot/Rust bundle authority, real seeded PPO/SAC, executable randomization, robustness evaluation, deterministic ONNX export, and a required clean source-bound smoke. P7-008 browser playback remains protected. P7-010's real CPU MuJoCo/MJX float64 feasibility harness is protected through PR #66/`0614272`; clean artifact `8337556569` passes parity and measures native MuJoCo about 4.9x faster than CPU-backed MJX on the reference row, but remains decision-ineligible until exact D12 quad/rover/legged accelerator, budget, cost, parity, and 3x cost-normalized evidence exists. P7-011 is protected through PR #68/`9131289`: clean artifact `8340587390` proves lease-fenced exact object-backed one-click delivery, 22 migrations, exact retained-byte browser execution, and all declared browser tiers at controlled S3-compatible sandbox maturity. The P7-014 waypoint slice is protected through PR #70/`f220d25`: clean artifact `8342801418` binds D41 task-v2 Y-up/hash authority to sequential estimator-only waypoint execution, Studio playback, and honest dual-task smoke. P7-012 is in implementation: D42's tensor-v2/bundle-v2/task-v3 correction, exact v1 read compatibility, frozen estimator-only distillation/randomized-PPO recipe, resumable evidence runner, and passing local exact-seed M2 Pro hover/waypoint diagnostics are present; D43 records honest CPU selection on a GPU-capable host and energy/cost nonclaims. P7-012 still requires protected implementation plus clean interrupt/resume evidence. Deployed Modal/GPU operations, offline fine-tune, rover/legged tasks, the final MJX decision, production storage operations, and EXT-003 remain open as P7-009..014/OPS-006.)* | 4 wk |
...
Exit criteria:
- [~] A trained hover + waypoint policy flies the twin in-browser from a one-click job *(controlled real hover training and digest-bound browser inference are protected through P7-003/P7-008. P7-011 is closed through PR #68/`9131289`: one-click admitted-model create/poll, lease-fenced exact object materialization, authenticated retained-model fetch, and double-verified playback pass protected PostgreSQL/S3-compatible/browser acceptance. P7-014's waypoint slice is protected through PR #70/`f220d25`. P7-012's current candidate produces locally passing task-v3/tensor-v2 hover and waypoint ONNX policies and retains tensor-v1 playback, but protected clean-source evidence and end-to-end retained-object execution of those passing policies remain before this exit closes.)*
...
- [~] Hover-class task to passing scorecard overnight on one consumer GPU *(exact-seed local M2 Pro diagnostics pass unchanged baseline/mass/Kv/wind gates under D42/D43; protect the implementation, execute intentional interruption/resume on that clean revision, download/verify both policies, and reconcile before closure)*
