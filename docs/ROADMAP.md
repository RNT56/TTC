...
- [~] At least one real rover and one real legged task execute through the sovereign trainer *(D44's local candidate runs rover `line-follow` and quadruped `walk-to-target` from admitted contract-derived physics, estimator/encoder-only tensors, bounded torque, task-specific robustness, exact ONNX, and four-task smoke. Exact-head/protected checks and a retained clean-source smoke remain before P7-014 closes; Studio ground playback and field transfer are separate.)*
