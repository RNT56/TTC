...
| P9 Co-design | ◑ *(2026-07-16 D59 candidate: the deterministic 200-candidate fixture and admitted-point Studio flow remain; a new exact-snapshot controlled smoke now executes three-to-nine inline multirotor variants through sovereign native validation, deterministic real Rapier 0.33.0, and pinned MuJoCo 3.9.0, with strict lineage/readback and tier 3 held. Local proof admits 2/3, returns one Pareto point, and passes the 50 ms release-native tier-0 SLO; all 41 required repository gates pass under Python 3.12.7 with 233 worker tests. Protected-main evidence, actual CMA-ES/Optuna, catalog choices, ≥3 Pareto points, 200 engine candidates overnight, trained finalists, and provider proof remain open.)* | 4 wk |
...
Exit criteria:
- [~] "Lightest quad for this course under constraints" returns ≥ 3 admitted Pareto points overnight *(the keyless 200-candidate constrained-course shape proof remains live. D59 adds a controlled three-candidate engine smoke with two admitted candidates and one recomputed Pareto point, but it is neither a course-conditioned ≥3-point result nor an overnight optimizer run.)*
- [~] Tier-0 candidate evaluation < 50 ms native; 200-candidate CMA-ES generation overnight at tier 2 *(D59's repeated release-binary runs pass the sovereign native 50 ms SLO and exercise real Rapier 0.33.0 plus pinned MuJoCo 3.9.0 for three candidates. Actual CMA-ES/Optuna, 200 engine candidates, provider scheduling, and retained overnight evidence remain open.)*
- [~] Every returned point is a fully admitted contract (validator as constraint oracle) *(D59's controlled engine result recomputes every replace-only candidate snapshot and native-evidence hash, admits through the sovereign validator/Rapier/MuJoCo ladder, and derives Pareto only from admitted points. The candidate still needs protected-main proof; full optimizer breadth and trained finalists remain open.)*
