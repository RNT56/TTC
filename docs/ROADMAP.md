...
| P9 Co-design | ◑ *(2026-07-17: D59's exact-snapshot three-to-nine-candidate sovereign native/Rapier/MuJoCo smoke is protected through PR #106/`fae00c5`; it admits 2/3, returns one Pareto point, and passes the 50 ms tier-0 SLO within all 41 protected gates. D60's 42-gate-green proposal plan is protected through PR #109/`71e7217`: 100 pinned CMA-ES plus 100 pinned Optuna TPE proposals retain deterministic hashes and no physical/engine feedback. D61 is a 43-gate-green local implementation candidate: its separate checkpointed format consumes the exact 200 hashes, persists every candidate, demonstrates 7 + zero-dispatch cancel + 193 resume, evaluates 200 native and 125 eligible Rapier/MuJoCo rows, admits 89, derives four Pareto points, and selects three tier-3-held finalists. The first run was from a dirty development worktree, so a clean exact-source repeat and protected evidence remain pending. Catalog choices, actual tier-3 training, retained overnight/provider billing proof, and external acceptance remain open.)* | 4 wk |
...
Exit criteria:
- [~] "Lightest quad for this course under constraints" returns ≥ 3 admitted Pareto points overnight *(D61's local diagnostic exact-hash batch returns four independently derived engine-admitted Pareto points, but it is neither clean/protected yet nor course-conditioned/overnight/provider evidence. Those gates remain open.)*
- [~] Tier-0 candidate evaluation < 50 ms native; 200-candidate CMA-ES generation overnight at tier 2 *(D59's protected release-binary runs pass the separate sovereign native 50 ms SLO. D60 produces 200 real-algorithm proposals. D61 locally consumes all 200 through native and 125 eligible Rapier/MuJoCo rows with durable recovery, but a clean/protected batch plus retained overnight/provider schedule and billing remain open.)*
- [~] Every returned point is a fully admitted contract (validator as constraint oracle) *(D59 protects the small exact-snapshot constraint-oracle seam. D61's completed local batch derives its front only from fully admitted sovereign rows and keeps partial fronts empty; clean/protected proof, catalog choices, and actual trained finalists remain open.)*
...
D61 is the separate local engine consumer. Internal
...
with tier 3 explicitly held. A development run completed 200 native and 125 eligible
Rapier/MuJoCo evaluations, admitted 89, and returned four Pareto points after a
7 + cancel + 193 recovery sequence. It records measured local runtime but no
provider cost/currency, energy, or overnight authority. That first run is diagnostic
because the implementation worktree was dirty; all 43 local gates pass with 242
worker tests, while a clean exact-source repeat and protected PR/post-merge proof
remain required. Catalog choices, actual trained-
finalist scorecards, provider/overnight operations, and external acceptance still
prevent P9 closure.
