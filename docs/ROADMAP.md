...
| P7 Training service | ◑ *(2026-07-15: task specs, obs/action derivation, domain randomization, curriculum metadata, fixture train.policy/train.sysid-fit jobs, external SB3 scorecard re-gating, ONNX headers/blob-linked policy artifacts, and P7-008's protected browser playback are live. A controlled exact-pinned CPU MuJoCo/SB3 hover runtime with admitted snapshot/Rust bundle authority, real PPO/SAC, robustness evaluation, deterministic ONNX export, and a required source-bound smoke is implemented as the P7-003 protected-evidence candidate. Overnight passing hover/waypoint proof, one-click object-backed delivery, deployed Modal/GPU evidence, offline fine-tune, MJX proof, and EXT-003 remain open.)* | 4 wk |
...
Exit criteria:
- [~] A trained hover + waypoint policy flies the twin in-browser from a one-click job *(real digest-bound browser inference is protected and a real locally trained hover ONNX artifact is produced by the P7-003 candidate; one-click queue/object transport, passing-policy proof, waypoint coverage, and protected end-to-end evidence remain open)*
