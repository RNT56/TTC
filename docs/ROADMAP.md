...
| P7 Training service | ◑ *(2026-07-15: task specs, obs/action derivation, domain randomization, curriculum metadata, fixture train.policy/train.sysid-fit jobs, external SB3 scorecard re-gating, ONNX headers/blob-linked policy artifacts, and a local P7-008 candidate that runs a digest-bound opset-18 hover model through lazy exact ONNX Runtime Web 1.27.0/WASM against Rust estimator observations; protected browser/CI evidence is pending, while live SB3/MuJoCo training, object-backed external models, offline fine-tune, and MJX proof remain open)* | 4 wk |
