...
maturity; real adapter/device, suspend, Studio import, and field proof remain owned by
the adjacent P8 tasks.

D51's current candidate advances P8-003 with a read-only import-verification seam.
...
pass; exact protected PR/post-merge evidence remains the next gate. After
that, the local dependency order is recorder start/stop controls and gateway-owned
import/materialization, while real adapter/device and suspend execution proceed only
under the separate D12 lab lane.
